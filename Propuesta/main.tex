\documentclass[12pt]{article}
\usepackage{float}
\usepackage{xcolor}
\usepackage{newpxtext,euler}
\usepackage[OT1]{fontenc}
\usepackage[spanish]{babel}
\usepackage{amsfonts,amsmath,amssymb,amsthm}
\usepackage{hyperref}
\usepackage{geometry}
\usepackage{bm} % for \bm
\usepackage{fixmath}

\newcommand\N{\ensuremath{\mathbb{N}}}
\newcommand\R{\ensuremath{\mathbb{R}}}
\newcommand\Z{\ensuremath{\mathbb{Z}}}
\renewcommand\O{\ensuremath{\emptyset}}
\newcommand\Q{\ensuremath{\mathbb{Q}}}
\newcommand\C{\ensuremath{\mathbb{C}}}
\newcommand\T{\mathbb{T}}
\renewcommand{\epsilon}{\varepsilon}
\renewcommand{\hat}{\widehat}
\newcommand\jk{\langle k\rangle}


\setlength{\parindent}{0pt}


\input{boxes}
\usepackage{lipsum}
 \geometry{
 a4paper,
 total={170mm,260mm},
 left=20mm,
 top=15mm,
 }
 
\title{\vspace{-2cm}\par\noindent\rule{16cm}{1pt}\large
\\\bfseries Grupo de Trenzas y Espacios de Configuración
\vspace{-0.34cm}\par\noindent\hspace{0.15cm}\rule{16cm}{1pt}
\vspace{-0.6cm}
}
\author{\small \bfseries \quad \quad\small Edgar Santiago Ochoa Quiroga\\ \small \quad \quad \quad \quad \quad \texttt{eochoa@unal.edu.co}\quad\quad \quad\\}

\usepackage{titling}
\predate{\hspace{6.24cm}\small}
\postdate{}

\begin{document}
\maketitle
\begin{abstract}
    Este trabajo estudia el concepto del grupo de trenzas desde múltiples perspectivas, geométrica, algebraica, como espacios de configuración y mas que surgen de estas. Posterior a estas mostramos como cada una de estas perspectivas resultan equivalentes y se mencionaran algunos resultados interesantes derivados del concepto de trenzas, intentando hacer gran enfasis en la vista geométrica. 
\end{abstract}
\section{Introducción}
La primera aparición con mención propia de los grupos de trenzas se las debemos a Emil Artin \cite{Artin1925}, quien en 1925 los introdujo para modelar como se entrelazaban múltiples cuerdas en un espacio euclidiano 3-dimensional, estas cuerdas es lo que conocemos como trenzas. No es sorpresa que el estudio de estos objetos tan naturales en la naturaleza resulte de interés debido a su clara conexión con la teoría nudos.

En este proyecto cubriremos el primer capitulo de \cite{KasselTuraev2008}, con la intención de introducir al lector a los grupos de trenzas y todas sus posibles perspectivas, complementar pruebas, presentar diagramas diferentes y en general dar una vista mas aterrizada a entender estas estructuras tan interesantes. Presentaremos las definiciones elementales y multiples perspectivas para luego revisar resultados de equivalencia y finalizar con algunos hechos bien conocidos en la literatura pero que resultan interesantes para un nuevo lector.

\section{Diferentes acercamientos a las Trenzas}
\subsection{Las Trenzas Geométricas}
\input{Trenzas Geometricas/Geometricas}

\subsection{El Grupo de Trenzas de Artin}
\input{Trenzas Algebraicas/Algebra}
\subsection{Grupo de Trenzas Puro}
\input{Trenzas Algebraicas/Puras}
\subsection{Espacios de Configuración}
\input{Trenzas Topologicas/Confspace}
\subsection{Automorfismos de Trenzas sobre Grupos Libres}
\input{Trenzas Algebraicas/Automor}
\subsection{Trenzas y Homeomorfismos}
\input{Trenzas Topologicas/MCG}
\section{Resultados de Equivalencia}
Durante este capitulo probaremos la equivalencias respectivas entre las definiciones presentadas en el anterior capitulo y algunos hechos derivados de estas equivalencias.
\subsection{El grupo de trenzas geométrico}
\input{Equiv1}
\subsection{El grupo de Trenzas puro y los espacios de configuración }
\input{Equiv2}
\subsection{El grupo de automorfismos y Mapping Class Group}
\input{Equiv3}
\section{Resultados estándar conocidos.}
\subsection{El grupo $B_3$}
\input{B3}
\subsection{Propiedades de $B_n$ y $P_n$}
\input{res2}
\begin{thebibliography}{9}

\input{ref}

\end{thebibliography}

\end{document}


